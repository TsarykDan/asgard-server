...
class MainGameScreen(ColoredScreen):
    ...
    def __init__(self, **kwargs):
        ...
        self.header = BoxLayout(orientation='horizontal', size_hint_y=0.08, padding=10, spacing=8)
        self.info_label = Label(text="", halign="left", valign="middle", color=COLOR_GOLD, bold=True)
        ...
        self.tabs_nav = BoxLayout(orientation='horizontal', size_hint_y=0.08, spacing=3)
        tabs = [
            ("Чат", 'chat'), 
            ("Карта", 'map_city'),
            ("Політична", 'map_political'),
            ("ЛС", 'pm'), 
            ("Перекази", 'transfer'), 
            ("Казино", 'casino'), 
            ("Магазин", 'shop'), 
        ...
        for title, key in tabs:
            btn = AsgardButton(text=title, bg_color=COLOR_TAB_BG, text_color=COLOR_TEXT_WHITE, radius=[5])
            btn.bind(on_press=lambda instance, k=key: self.show_tab(k))
            self.tabs_nav.add_widget(btn)
            
        self.main_layout.add_widget(self.tabs_nav)
        
        self.content_area = BoxLayout(orientation='vertical', size_hint_y=0.84, padding=10)
    ...
    def update_header(self):
        ...
        self.info_label.text = (
            ...
            f"[Капітал Банку: {bank_capital:.2f}] | [Баланс: {self.user_data['balance']:.2f} юнітів]"
        )
        ...
        if role == "Адмін":
            ...
            self.btn_panel.text = "Пульт Адміна"
            self.btn_panel.bg_color = COLOR_ADMIN_RED
        elif role == "Король":
            self.btn_panel.opacity = 1
            self.btn_panel.disabled = False
            self.btn_panel.text = "Пульт Короля"
    ...
    def show_tab(self, tab_name):
        ...
        if tab_name == 'chat': 
            self.build_chat_tab()
        elif tab_name == 'map_city':
            self.build_map_city_tab()
        elif tab_name == 'map_political':
            self.build_map_political_tab()
        elif tab_name == 'pm':
            self.build_pm_tab()
    ...
    def build_chat_tab(self):
        ...
        self.order_board = Label(
            ...
            size_hint_y=0.12, color=COLOR_GOLD, bold=True, halign="center", valign="middle", markup=True
        )
        self.order_board.bind(size=self.order_board.setter('text_size'))
        chat_layout.add_widget(self.order_board)
        
        self.chat_scroll = ScrollView(size_hint_y=0.65)
        ...
        input_layout = BoxLayout(orientation='horizontal', size_hint_y=0.13, spacing=5)
        ...
        btn_complaint = AsgardButton(
            text="[УВАГА] Подати скаргу на порушника закону", size_hint_y=0.1, 
    ...
    # --- ВКЛАДКА: КАРТА МІСТА ---
    def build_map_city_tab(self):
        layout = BoxLayout(orientation='vertical', spacing=8)
        layout.add_widget(Label(
            text="=== КАРТА МІСТА АЗГАРДУ ===", 
            font_size=18, color=COLOR_GOLD, bold=True, size_hint_y=0.08
        ))
        try:
            map_img = Image(source=resource_path('map_city.jpg'), allow_stretch=True, keep_ratio=True, size_hint_y=0.92)
            layout.add_widget(map_img)
        except Exception:
            layout.add_widget(Label(text="Зображення map_city.jpg не знайдено!", color=COLOR_RED))
            
        self.content_area.add_widget(layout)

    # --- ВКЛАДКА: ПОЛІТИЧНА КАРТА ---
    def build_map_political_tab(self):
        layout = BoxLayout(orientation='vertical', spacing=8)
        layout.add_widget(Label(
            text="=== ПОЛІТИЧНА КАРТА АЗГАРДУ ТА СУСІДІВ ===", 
            font_size=18, color=COLOR_GOLD, bold=True, size_hint_y=0.08
        ))
        try:
            map_img = Image(source=resource_path('map_political.jpg'), allow_stretch=True, keep_ratio=True, size_hint_y=0.92)
            layout.add_widget(map_img)
        except Exception:
            layout.add_widget(Label(text="Зображення map_political.jpg не знайдено!", color=COLOR_RED))
        ...
        try:
            ...
            for msg in messages:
                ...
                lbl = Label(
                    ...
                    size_hint_y=None, height=35, halign="left", valign="middle", markup=True
                )
    ...
    def build_transfer_tab(self):
        ...
        layout.add_widget(Label(
            ...
            font_size=20, color=COLOR_GOLD, bold=True, size_hint_y=0.12
        ))
        ...
        self.tr_status_label = Label(
            ...
            font_size=15, color=COLOR_TEXT_WHITE, bold=True, halign="center", size_hint_y=0.46
        )
    ...
    def build_casino_tab(self):
        ...
        layout.add_widget(Label(
            ...
            font_size=20, color=COLOR_GOLD, bold=True, size_hint_y=0.1
        ))
        ...
        layout.add_widget(Label(text=info_text, markup=True, halign="center", size_hint_y=0.25))
        ...
        self.casino_result_label = Label(
            ...
            font_size=16, color=COLOR_TEXT_WHITE, bold=True, halign="center", size_hint_y=0.25
        )
    ...
    def build_shop_tab(self):
        ...
        grid.add_widget(Label(text="=== ДЕРЖАВНИЙ МАГАЗИН АЗГАРДУ ===", font_size=22, color=COLOR_GOLD, bold=True, size_hint_y=None, height=40))
        grid.add_widget(Label(text="--- ВІП СТАТУСИ ---", font_size=16, color=COLOR_GOLD, bold=True, size_hint_y=None, height=30))
        ...
        for level, label, price, desc in vips:
            ...
            btn = AsgardButton(text="Купити", bg_color=COLOR_GOLD_BTN, size_hint_x=0.3, radius=[5])
        ...
        vip4_btn = AsgardButton(text="За досягнення", bg_color=(0.2, 0.2, 0.2, 1), text_color=(0.5, 0.5, 0.5, 1), size_hint_x=0.3, radius=[5], disabled=True)
        vip4_row.add_widget(vip4_btn)
        grid.add_widget(vip4_row)
        
        grid.add_widget(Label(text="--- ДЕРЖАВНІ ДОЗВОЛИ ---", font_size=16, color=COLOR_GOLD, bold=True, size_hint_y=None, height=30))
        ...
        for key, label, price in permits:
            row = BoxLayout(orientation='horizontal', size_hint_y=None, height=45, spacing=10)
            row.add_widget(Label(text=label, color=COLOR_TEXT_WHITE, size_hint_x=0.5, halign='left'))
            row.add_widget(Label(text=f"{price} Ю", color=COLOR_GOLD, size_hint_x=0.2, bold=True))
            btn = AsgardButton(text="Придбати", bg_color=COLOR_GOLD_BTN, size_hint_x=0.3, radius=[5])
    ...
    def build_market_tab(self):
        ...
        top_bar.add_widget(Label(text="=== РИНОК АЗГАРДУ ===", font_size=18, color=COLOR_GOLD, bold=True, halign="left"))
        btn_sell = AsgardButton(text="Виставити предмет", size_hint_x=0.4, bg_color=COLOR_GOLD_BTN, radius=[5])
        ...
        try:
            ...
            for item in items:
                ...
                if seller == current_user:
                    btn_action = AsgardButton(text="Забрати", bg_color=(0.5, 0.3, 0.1, 1), size_hint_x=0.35, radius=[5])
                    btn_action.bind(on_press=lambda instance, i_id=item_id: self.cancel_market_item(i_id))
                elif current_role == "Адмін":
                    btn_action = AsgardButton(text="Видалити (Admin)", bg_color=COLOR_ADMIN_RED, text_color=COLOR_TEXT_WHITE, size_hint_x=0.35, radius=[5])
                    btn_action.bind(on_press=lambda instance, i_id=item_id: self.cancel_market_item(i_id, is_admin_override=True))
                else:
                    btn_action = AsgardButton(text="Купити", bg_color=(0.1, 0.5, 0.3, 1), text_color=COLOR_TEXT_WHITE, size_hint_x=0.35, radius=[5])
    ...
    def build_citizens_tab(self):
        ...
        grid.bind(minimum_height=grid.setter('height'))
        grid.add_widget(Label(text="=== РЕЄСТР ЖИТЕЛІВ АЗГАРДУ ===", font_size=22, color=COLOR_GOLD, bold=True, size_hint_y=None, height=40))
        ...
        try:
            ...
            for u in users:
                ...
                lbl = Label(text=card_text, markup=True, size_hint_y=None, height=80, halign="left", valign="middle")
                lbl.bind(size=lbl.setter('text_size'))
                grid.add_widget(lbl)
    ...
    def build_rules_tab(self):
        ...
        title = Label(text="=== КОДЕКС ЗАКОНІВ АЗГАРДУ ===", font_size=22, color=COLOR_GOLD, bold=True, size_hint_y=None, height=40)
    ...
    def open_control_panel(self, instance):
        ...
        popup_layout.add_widget(Label(text=title_text, font_size=18, color=title_color, bold=True, size_hint_y=None, height=35))
        ...
        if is_admin:
            ...
            btn_delete_acc = AsgardButton(text="ВИДАЛИТИ АКАУНТ", bg_color=(0.9, 0.1, 0.1, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        content_grid.add_widget(Label(text="Державні скарги громадян:", size_hint_y=None, height=25, color=COLOR_RED, bold=True))
        ...
        content_grid.add_widget(Label(text="Дисциплінарний Комітет (Бан/Розбан):", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        ...
        btn_ban = AsgardButton(text="Забанити", bg_color=(0.7, 0.1, 0.1, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        btn_unban = AsgardButton(text="Розбанити", bg_color=(0.1, 0.5, 0.1, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        content_grid.add_widget(Label(text="Державна Казначея (Юніти):", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        ...
        btn_give = AsgardButton(text="+ Дати", bg_color=(0.1, 0.5, 0.3, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        btn_take = AsgardButton(text="- Забрати", bg_color=(0.5, 0.3, 0.1, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        btn_set = AsgardButton(text="= Встановити", bg_color=(0.2, 0.4, 0.6, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        content_grid.add_widget(Label(text="Надання ВІП-Рангів (0-4):", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        ...
        btn_set_vip = AsgardButton(text="Встановити ВІП", bg_color=(0.1, 0.5, 0.7, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        content_grid.add_widget(Label(text="Керування Дозволами (продаж / тер / їжа / зброя / інстр):", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        ...
        btn_give_perm = AsgardButton(text="Дати дозвіл", bg_color=(0.1, 0.6, 0.3, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        btn_revoke_perm = AsgardButton(text="Забрати", bg_color=(0.7, 0.2, 0.2, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        content_grid.add_widget(Label(text="Оголошення Наказів / Завдань:", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        ...
        content_grid.add_widget(Label(text="Збори та Державний Порядок:", size_hint_y=None, height=25, color=COLOR_GOLD, bold=True))
        btn_tax = AsgardButton(text="Зібрати загальний податок з усіх підданих (По 50 юнітів)", size_hint_y=None, height=40, bg_color=(0.5, 0.4, 0.1, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
        ...
        btn_close = AsgardButton(text="Закрити пульт керування", size_hint_y=None, height=40, bg_color=(0.2, 0.2, 0.2, 1), text_color=COLOR_TEXT_WHITE, radius=[5])
